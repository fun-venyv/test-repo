    ...
    try {
        const res = await this.Http.get({ url: `/applications/public?application_ids=${appId}` });  
            ...
            async ACHIEVEMENT(q, t) {
                ...
                if (chan) {
                    this.Logger.log(`[Задача] Попытка спуфинга heartbeat для "${t.name}"...`, 'info');
                    ...
                    while (cur < t.target && this.RUNTIME.running) {
                        try {
                            ...
                            const err = this.ErrorHandler.classify(e);
                            if (err.isClientError) {
                                this.Logger.log(`[Достижение] Heartbeat отклонен (HTTP ${err.status}). Переход в пассивный режим.`, 'warn');
                                break;
                            }
                            if (failCount >= this.SYS.MAX_TASK_FAILURES) {
                                this.Logger.log(`[Достижение] Слишком много ошибок. Переход в пассивный режим.`, 'warn');
                                break;
                            }
                        }
                ...
                if (!this.RUNTIME.running) return;
                this.Logger.log(`[Задача] Пропускаем "${t.name}" — heartbeat отклонён.`, 'warn');
            ...
            async ACTIVITY(q, t) {
                ...
                try {
                    ...
                    this.Logger.log(`[Задача] Ошибка поиска канала ACTIVITY: ${e.message}`, 'debug');
                }

                if (!chan) {
                    return this.failTask(q, t, 'Голосовой канал не найден');
                }
                ...
                while (cur < t.target && this.RUNTIME.running) {
                    try {
                        ...
                        if (cur >= t.target) {
                            ...
                            catch (e) { this.Logger.log(`[ACTIVITY] Ошибка финального heartbeat: ${e?.message}`, 'debug'); }
                            break;
                        }
                    } catch (e) {
                        failCount++;
                        const err = this.ErrorHandler.classify(e);
                        if (err.isClientError) {
                            this.Logger.log(`[Задача] Квест активности недоступен (HTTP ${err.status}). Пропускаем.`, 'warn');
                            return this.failTask(q, t, `Ошибка клиента ${err.status}`);
                        }
                        if (failCount >= this.SYS.MAX_TASK_FAILURES) {
                            return this.failTask(q, t, 'Слишком много сетевых ошибок');
                        }
                        this.Logger.log(`[Задача] Ошибка heartbeat ACTIVITY (${failCount}/${this.SYS.MAX_TASK_FAILURES}): ${err.message}`, 'debug');
                    }
